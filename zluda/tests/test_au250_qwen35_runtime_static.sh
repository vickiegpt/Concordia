...
zluda_builder="${repo_root}/zluda/build.rs"
...
grep -Fq 'HETGPU_CUDART_ABI_MAJOR=13' "${builder}"
grep -Fq 'libhetgpu_cuda_shim.so' "${builder}"
grep -Fq 'cudart_shim' "${builder}"
grep -Fq 'cargo:rerun-if-env-changed=HETGPU_CUDART_ABI_MAJOR' "${zluda_builder}"
grep -Fq 'libcudart.so.{cudart_abi_major}' "${zluda_builder}"
...
grep -Fq 'libhetgpu_cuda_shim.so' "${iq1s_runner}"
grep -Fq '"${cudart_shim}:${libnvcuda}"' "${iq1s_runner}"
