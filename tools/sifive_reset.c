...
static const uint64_t SIFIVE_TOP_REG_OFF = 0x201000ULL;
static const uint64_t CORE_RST_OFF[SIFIVE_CORE_NUM] = {0x14, 0x18, 0x1c, 0x20};
...
static void usage(const char *argv0) {
    fprintf(stderr,
        ...
        "  %s --core-reset --cores-are-wfi [--sifive-mask 0xf] [--core-mask 0xf] [--reset-vec 0x30080000]\n"
        "  %s --sys-cold-reset --cores-are-wfi [--sifive-mask 0xf] [--reset-vec 0x30080000]\n"
        "  %s --release [--sifive-mask 0xf] [--core-mask 0xf] [--reset-vec 0x30080000]\n"
        "\n"
        "Safety:\n"
        "  Reset operations require --cores-are-wfi. Do not use them while any SIFIVE core is running.\n"
...
static uint64_t top_addr(unsigned sifive_id, uint64_t off) {
    return sifive_base[sifive_id] + SIFIVE_TOP_REG_OFF + off;
}
...
static void print_status(int memfd, const struct opts *o) {
    ...
    for (unsigned p = 0; p < SIFIVE_CLUST_NUM; ++p) {
        ...
        printf("SIFIVE%u top=0x%016" PRIx64 "\n", p, top_addr(p, 0));
...
static void run_reset(int memfd, const struct opts *o) {
    if ((o->core_reset || o->sys_cold_reset) && !o->cores_are_wfi) {
        fprintf(stderr, "refusing reset: pass --cores-are-wfi only after confirming all selected SIFIVE cores are in WFI\n");
        exit(2);
    }
