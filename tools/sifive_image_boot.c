...
static const uint64_t sifive_base[SIFIVE_CLUST_NUM] = {
    0x38100000ULL, 0x38500000ULL, 0x39100000ULL, 0x39500000ULL,
};

static const uint64_t LX500_SIFIVE_TOP_ABP_CRG_BASE = 0x200000ULL;
static const uint64_t LX500_SIFIVE_TOP_ABP_CFG_BASE = 0x201000ULL;
static const uint64_t LX500_SIFIVE_TOP_ABP_MBOX_RAM_BASE = 0x210000ULL;
static const uint64_t LX500_SIFIVE_TOP_ABP_MBOX_DB_BASE = 0x214000ULL;
static const uint64_t LX500_SIFIVE_CFG_CORE_RESET[SIFIVE_CORE_NUM] = {0x14, 0x18, 0x1c, 0x20};
static const uint64_t LX500_SIFIVE_CFG_SYS_RESET = 0x24;
static const uint64_t LX500_SIFIVE_CFG_FORCE_RESETPC_RELOAD = 0x28;
static const uint64_t LX500_SIFIVE_CFG_RST_VECTOR_0_LOW = 0x6c;
static const uint64_t LX500_SIFIVE_CFG_RST_VECTOR_0_HIGH = 0x70;
static const uint64_t LX500_SIFIVE_CFG_SECURE_TIEOFF = 0xbc;
static const uint64_t LX500_SIFIVE_CFG_MAM_CRM = 0xc4;
static const uint64_t LX500_SIFIVE_MBOX_RAM_SIZE = 0x4000ULL;

struct sifive_base_cmd {
    uint64_t reserved0;
    uint64_t reserved1;
    uint64_t base;
    uint64_t size;
...
static void usage(const char *argv0) {
    fprintf(stderr,
        ...
        "\n"
        "options:\n"
        "  --sifive-mask 0xf        select SIFIVE clusters\n"
        "  --core-mask 0xf        select cores inside each SIFIVE\n"
        "  --init-crg             run the vendor 1100MHz ref-to-PLL CRG init sequence\n"
        "  --send-base-cmd        send the loader base/size command through SIFIVE mailbox RAM\n"
        ...
        "  1. copy raw lx500_sifive.bin firmware image to SIFIVE-visible DDR at --load-addr\n"
        ...
        "\n"
        "Safety:\n"
        "  Reset operations require --cores-are-wfi. Do not reset running SIFIVE cores.\n",
...
static uint64_t cfg_addr(unsigned sifive_id, uint64_t off) {
    return sifive_base[sifive_id] + LX500_SIFIVE_TOP_ABP_CFG_BASE + off;
}

static uint64_t crg_addr(unsigned sifive_id, uint64_t off) {
    return sifive_base[sifive_id] + LX500_SIFIVE_TOP_ABP_CRG_BASE + off;
}

static uint64_t mbox_ram_addr(unsigned sifive_id, uint64_t off) {
    return sifive_base[sifive_id] + LX500_SIFIVE_TOP_ABP_MBOX_RAM_BASE + off;
}

static uint64_t mbox_db_addr(unsigned sifive_id, uint64_t off) {
    return sifive_base[sifive_id] + LX500_SIFIVE_TOP_ABP_MBOX_DB_BASE + off;
}
...
static void print_status(int memfd, const struct opts *o) {
    ...
    for (unsigned p = 0; p < SIFIVE_CLUST_NUM; ++p) {
        if ((o->sifive_mask & (1u << p)) == 0) continue;
        printf("SIFIVE%u cfg=0x%016" PRIx64 " crg=0x%016" PRIx64 "\n",
               p,
               sifive_base[p] + LX500_SIFIVE_TOP_ABP_CFG_BASE,
               sifive_base[p] + LX500_SIFIVE_TOP_ABP_CRG_BASE);
        printf("  SYS_RESET[0x24]       = 0x%08" PRIx32 "\n", mmio_read32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_SYS_RESET)));
        printf("  SECURE_TIEOFF[0xbc]   = 0x%08" PRIx32 "\n", mmio_read32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_SECURE_TIEOFF)));
        printf("  MAM_CRM[0xc4]         = 0x%08" PRIx32 "\n", mmio_read32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_MAM_CRM)));
        for (unsigned c = 0; c < SIFIVE_CORE_NUM; ++c) {
            printf("  CORE%u_RESET[0x%02" PRIx64 "]  = 0x%08" PRIx32 "\n",
                   c, LX500_SIFIVE_CFG_CORE_RESET[c], mmio_read32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_CORE_RESET[c])));
            printf("  CORE%u_RST_VEC        = 0x%08" PRIx32 "%08" PRIx32 "\n",
                   c,
                   mmio_read32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_RST_VECTOR_0_HIGH + 8ULL * c)),
                   mmio_read32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_RST_VECTOR_0_LOW + 8ULL * c)));
        }
    }
}
...
static void program_cfg(int memfd, const struct opts *o, unsigned p) {
    ...
    for (unsigned c = 0; c < SIFIVE_CORE_NUM; ++c) {
        ...
        mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_RST_VECTOR_0_LOW + 8ULL * c), lo, o->dry_run);
        mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_RST_VECTOR_0_HIGH + 8ULL * c), hi, o->dry_run);
    }
    if (o->have_secure_tieoff) {
        mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_SECURE_TIEOFF), o->secure_tieoff, o->dry_run);
    }
    if (o->have_mam_crm) {
        mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_MAM_CRM), o->mam_crm, o->dry_run);
    }
    mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_FORCE_RESETPC_RELOAD), o->core_mask & 0xfu, o->dry_run);
    mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_FORCE_RESETPC_RELOAD), 0, o->dry_run);
}
...
static void init_crg_1100_ref2pll(int memfd, unsigned p, bool dry_run) {
    ...
    if (!dry_run)
        printf("SIFIVE%u CRG init done pll_status=0x%08" PRIx32 "\n",
               p, mmio_read32(memfd, crg_addr(p, 0x0)));
}

static void send_base_cmd(int memfd, const struct opts *o, unsigned p) {
    struct sifive_base_cmd cmd = {
        .base = o->load_addr,
        .size = o->reserved_size,
    };

    zero_phys(memfd, mbox_ram_addr(p, 0), (size_t)LX500_SIFIVE_MBOX_RAM_SIZE, o->dry_run);
    ...
    printf("SIFIVE%u base command base=0x%016" PRIx64 " size=0x%016" PRIx64 "\n",
           p, cmd.base, cmd.size);
}
...
static void boot_selected(int memfd, const struct opts *o) {
    ...
    for (unsigned p = 0; p < SIFIVE_CLUST_NUM; ++p) {
        if ((o->sifive_mask & (1u << p)) == 0) continue;
        printf("SIFIVE%u entry=0x%016" PRIx64 "\n", p, o->entry);

        if (o->core_reset) {
            for (unsigned c = 0; c < SIFIVE_CORE_NUM; ++c) {
                if ((o->core_mask & (1u << c)) == 0) continue;
                mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_CORE_RESET[c]), 0x3, o->dry_run);
            }
            if (o->init_crg)
            ...
            for (unsigned c = 0; c < SIFIVE_CORE_NUM; ++c) {
                if ((o->core_mask & (1u << c)) == 0) continue;
                mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_CORE_RESET[c]), 0x0, o->dry_run);
            }
        } else if (o->sys_cold_reset) {
            mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_SYS_RESET), 0x1, o->dry_run);
            if (o->init_crg)
                init_crg_1100_ref2pll(memfd, p, o->dry_run);
            program_cfg(memfd, o, p);
            mmio_write32(memfd, cfg_addr(p, LX500_SIFIVE_CFG_SYS_RESET), 0x0, o->dry_run);
            if (o->send_base_cmd)
                send_base_cmd(memfd, o, p);
            for (unsigned c = 0; c < SIFIVE_CORE_NUM; ++c) {
                if ((o->core_mask & (1u << c)) == 0) continue;
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (strcmp(argv[i], "--sifive-mask") == 0 && i + 1 < argc) o.sifive_mask = parse_u32(argv[++i], "sifive-mask");
    ...
    if (ops != 1 || (o.sifive_mask & ~0xfu) || (o.core_mask & ~0xfu)) {
        usage(argv[0]);
        return 2;
    }
