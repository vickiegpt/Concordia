...
int main(void) {
    const int n = 6144;
    const int nb = n / QK_K;
    block_iq1_s * iq1 = malloc(ggml_row_size(GGML_TYPE_IQ1_S, n));
    ...
    for (int trial = 0; trial < 1000; ++trial) {
        for (int block = 0; block < nb; ++block) {
            iq1[block].d = ggml_fp32_to_fp16(0.001f * (float) (1 + trial % 9));
            for (size_t i = 0; i < sizeof(iq1[block].qs); ++i) {
                iq1[block].qs[i] = (uint8_t) next_u32();
            }
            for (size_t i = 0; i < sizeof(iq1[block].qh) / sizeof(iq1[block].qh[0]); ++i) {
                iq1[block].qh[i] = (uint16_t) next_u32();
            }
        ...
        float generic = 0.0f;
        ggml_vec_dot_iq1_s_q8_K(n, &optimized, 0, iq1, 0, q8, 0, 1);
        ggml_vec_dot_iq1_s_q8_K_generic(n, &generic, 0, iq1, 0, q8, 0, 1);
    ...
    printf("iq1_s trials=1000 max_abs=%g max_rel=%g\n", max_abs, max_rel);
    free(q8);
    free(iq1);
    return 0;
}
