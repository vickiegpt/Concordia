...
usage() {
  cat >&2 <<'USAGE'
Usage:
  repack_lx500_sifive_nested_initramfs.sh INPUT_BIN INJECT_DIR OUTPUT_BIN

The LX500 SIFIVE Linux firmware image has an outer newc initramfs that contains
...
cpio_offset="$(parse_u64 "${LX500_SIFIVE_INITRAMFS_OFFSET:-0x4C4578}")"
outer_space_size="$(parse_u64 "${LX500_SIFIVE_INITRAMFS_LENGTH:-0x17d1e00}")"

if [[ "${LX500_SIFIVE_INITRAMFS_AUTODETECT:-0}" == "1" ]]; then
  cpio_offset="$(
    set +o pipefail
...
(
  cd "${outer_root}"
  # The SIFIVE loader path for the LX500 image is sensitive to the nested
  ...
  # repack can boot far enough for sifive->id but never complete probe.
  {
    printf '.\0'
...
if (( new_outer_size > outer_space_size )); then
  ...
  echo "rebuild the SIFIVE Linux image instead of in-place repacking" >&2
  exit 1
fi
