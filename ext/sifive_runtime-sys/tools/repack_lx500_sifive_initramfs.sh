...
usage() {
  cat >&2 <<'USAGE'
Usage:
  repack_lx500_sifive_initramfs.sh INPUT_BIN INJECT_DIR OUTPUT_BIN

INPUT_BIN is the vendor lanxin/lx500_sifive.bin image.
...
if (( new_cpio_size > old_cpio_size )); then
  ...
  echo "refusing to change the raw Linux image length; rebuild the SIFIVE Linux image instead" >&2
  exit 1
fi
