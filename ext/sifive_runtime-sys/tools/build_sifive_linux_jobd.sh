...
src="${SIFIVE_LINUX_JOBD_SRC:-${repo_root}/sifive_linux_jobd/hetgpu_sifive_jobd.c}"
out="${1:-${repo_root}/target/hetgpu_sifive_jobd}"
extra_cflags=()
extra_ldflags=()
linux_march="${SIFIVE_LINUX_MARCH:-rv64gcv_zbb_zfh_zvfh_zfbfmin_zvfbfmin_zvfbfwma_zvl1024b_xsfvfwmaccqqq}"

if [[ -n "${SIFIVE_LINUX_CFLAGS_EXTRA:-}" ]]; then
  read -r -a extra_cflags <<< "${SIFIVE_LINUX_CFLAGS_EXTRA}"
fi
if [[ -n "${SIFIVE_LINUX_LDFLAGS_EXTRA:-}" ]]; then
  read -r -a extra_ldflags <<< "${SIFIVE_LINUX_LDFLAGS_EXTRA}"
fi
...
if [[ "${SIFIVE_LINUX_STATIC:-1}" == "0" ]]; then
