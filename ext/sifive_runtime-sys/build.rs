...
fn main() {
    ...
    if let Some(path) = &spike_path {
        ...
        println!("cargo:warning=SIFIVE: Found Spike at: {}", path);
        ...
        println!("cargo:warning=SIFIVE: Spike not found. Set SPIKE_PATH or install spike.");
    }
    ...
    // SIFIVE configuration constants
    println!("cargo:rustc-env=SIFIVE_VLEN=512");
    println!("cargo:rustc-env=SIFIVE_DEFAULT_SEW=8");
}
