...
static ssize_t mbox_write(struct file *file, const char __user *buf, size_t len, loff_t *ppos)
{
	...
	} else if ((u64)*ppos >= SIFIVE2AP_RW_USER_OFF &&
		   (u64)*ppos < SIFIVE2AP_RW_USER_OFF + MBOX_SIZE) {
		u64 off = (u64)*ppos - SIFIVE2AP_RW_USER_OFF;
		if (!g_sifive2ap[minor] || off + n > MBOX_SIZE) {
			mutex_unlock(&g_lock);
			return -EINVAL;
		}
		memcpy_toio(g_sifive2ap[minor] + off, tmp, n);
	} else {
		if (!g_ap2sifive[minor] || (size_t)*ppos + n > MBOX_SIZE) {
			mutex_unlock(&g_lock);
			return -EINVAL;
		}
		memcpy_toio(g_ap2sifive[minor] + *ppos, tmp, n);
	}
	mb();
	mutex_unlock(&g_lock);
...
static ssize_t mbox_read(struct file *file, char __user *buf, size_t len, loff_t *ppos)
{
	...
	if ((u64)*ppos >= SHARED_DDR_USER_OFF &&
	...
	} else if ((u64)*ppos >= AP2SIFIVE_READ_USER_OFF &&
		   (u64)*ppos < AP2SIFIVE_READ_USER_OFF + MBOX_SIZE) {
		u64 off = (u64)*ppos - AP2SIFIVE_READ_USER_OFF;
		...
		if (!g_sifive2ap[minor] || *ppos >= MBOX_SIZE) {
			mutex_unlock(&g_lock);
			return 0;
		}
		if ((size_t)*ppos + n > MBOX_SIZE)
			n = MBOX_SIZE - (size_t)*ppos;
		memcpy_fromio(tmp, g_sifive2ap[minor] + *ppos, n);
	}
	mutex_unlock(&g_lock);
...
static loff_t mbox_llseek(struct file *file, loff_t off, int whence)
{
	...
	if (next < 0 ||
	    ((u64)next > MBOX_SIZE &&
	     ...
	     !((u64)next >= AP2SIFIVE_READ_USER_OFF &&
	       (u64)next <= AP2SIFIVE_READ_USER_OFF + MBOX_SIZE) &&
	     !((u64)next >= SIFIVE2AP_RW_USER_OFF &&
	       (u64)next <= SIFIVE2AP_RW_USER_OFF + MBOX_SIZE) &&
...
static int __init hetgpu_sifive_mbox_init(void)
{
	...
	pr_info("hetgpu_sifive_mbox: %u SIFIVE mailboxes, SRAM off=0x%llx size=0x%lx shared_ddr=0x%llx+0x%lx user_off=0x%llx debugfs=/sys/kernel/debug/%s/{shared_ddr_base,shared_ddr_size}\n",
...
MODULE_DESCRIPTION("Tiny AP2SIFIVE mailbox helper with kernel-allocated shared DDR export");
