...
jobd_log_present = "PACC_JOBD_LOG" in os.environ
jobd_log = os.environ.get("PACC_JOBD_LOG", "0").strip() or "0"
...
jobd_progress_completion = os.environ.get("PACC_JOBD_PROGRESS_COMPLETION", "0").strip() or "0"
jobd_loop_trace = os.environ.get("PACC_JOBD_LOOP_TRACE", "0").strip() or "0"
...
jobd_elf_fallback = os.environ.get("PACC_JOBD_ELF_FALLBACK", "").strip()
...
jobd_fork_elf_timeout = os.environ.get("PACC_JOBD_FORK_ELF_TIMEOUT_MS", os.environ.get("HETGPU_PACC_JOBD_FORK_ELF_TIMEOUT_MS", "")).strip()
...
if jobd_log_present:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_LOG={jobd_log}")
...
if jobd_progress_completion != "0":
    rcs_lines.append(f"export HETGPU_PACC_JOBD_PROGRESS_COMPLETION={jobd_progress_completion}")
...
if jobd_elf_fallback:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_ELF_FALLBACK={jobd_elf_fallback}")
...
if jobd_status_msync:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_STATUS_MSYNC={jobd_status_msync}")
if jobd_rms_debug:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_RMS_DEBUG={jobd_rms_debug}")
if jobd_rms_local_copy:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_RMS_LOCAL_COPY={jobd_rms_local_copy}")
if jobd_rms_rvv:
...
if jobd_mmvf_copy_io:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_MMVF_COPY_IO={jobd_mmvf_copy_io}")
...
if jobd_kernel_metadata_first:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_KERNEL_METADATA_FIRST={jobd_kernel_metadata_first}")
if jobd_fork_elf:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_FORK_ELF={jobd_fork_elf}")
if jobd_fork_elf_timeout:
    rcs_lines.append(f"export HETGPU_PACC_JOBD_FORK_ELF_TIMEOUT_MS={jobd_fork_elf_timeout}")
...
outer_offset = parse_u64_env("LX500_PACC_INITRAMFS_OFFSET")
outer_length = parse_u64_env("LX500_PACC_INITRAMFS_LENGTH")
if outer_offset is not None:
    if outer_offset < 0 or outer_offset >= len(image):
        raise SystemExit(f"LX500_PACC_INITRAMFS_OFFSET 0x{outer_offset:x} outside image size 0x{len(image):x}")
    if outer_length is None:
        outer_length = len(image) - outer_offset
    if outer_length < 0 or outer_offset + outer_length > len(image):
        raise SystemExit(
            f"LX500_PACC_INITRAMFS window off=0x{outer_offset:x} len=0x{outer_length:x} "
