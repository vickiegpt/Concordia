...
#define SHARED_DDR_CONTROL_BASE_OFF 0x100000ULL
#define SHARED_DDR_CONTROL_BYTES 0x2000ULL
...
struct pacc_control_write {
	u64 pacc_id; /* U64_MAX means current device minor. */
	u64 off;
	u64 len;
	u64 user_ptr;
	u64 flags;
...
static ssize_t shared_ddr_write_from_user(u64 ddr_off,
...
static ssize_t control_write_debugfs(struct file *file,
				     const char __user *buf,
				     size_t len,
				     loff_t *ppos)
{
	...
	if (*ppos < 0)
		return -EINVAL;
	pos = (u64)*ppos;
	if (pos >= PACC_COUNT * SHARED_DDR_CONTROL_BYTES)
		return -EINVAL;
	ddr_off = pos;
	if (ddr_off >= shared_ddr_size)
		return -EINVAL;
	if (len > shared_ddr_size - ddr_off)
		len = shared_ddr_size - ddr_off;
	if (len > PACC_COUNT * SHARED_DDR_CONTROL_BYTES - pos)
		len = PACC_COUNT * SHARED_DDR_CONTROL_BYTES - pos;

	ret = shared_ddr_write_from_user(ddr_off, buf, len);
	if (ret > 0)
		*ppos += ret;
	return ret;
}

static loff_t control_write_debugfs_llseek(struct file *file,
					   loff_t off,
					   int whence)
{
	loff_t next;
	...
	if (next < 0 || (u64)next > PACC_COUNT * SHARED_DDR_CONTROL_BYTES)
		return -EINVAL;
	file->f_pos = next;
	return next;
}

static const struct file_operations control_write_debugfs_fops = {
	.owner = THIS_MODULE,
	.write = control_write_debugfs,
	.llseek = control_write_debugfs_llseek,
};
...
static long mbox_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == PACC_IOC_CONTROL_WRITE) {
		...
		if (copy_from_user(&req, (void __user *)arg, sizeof(req)))
			return -EFAULT;
		pacc_id = req.pacc_id == ~0ULL ? minor : (unsigned int)req.pacc_id;
		if (pacc_id >= PACC_COUNT || req.len > SIZE_MAX)
			return -EINVAL;
		if (req.off > SHARED_DDR_CONTROL_BYTES ||
		    req.len > SHARED_DDR_CONTROL_BYTES - req.off)
			return -EINVAL;
		if (!req.len)
			return 0;
		rel = (u64)pacc_id * SHARED_DDR_CONTROL_BYTES + req.off;
		if (rel >= shared_ddr_size || req.len > shared_ddr_size - rel)
			return -EINVAL;
		ubuf = (const char __user *)u64_to_user_ptr(req.user_ptr);
		wrote = shared_ddr_write_from_user(rel, ubuf, (size_t)req.len);
		return wrote == (ssize_t)req.len ? 0 : (wrote < 0 ? wrote : -EIO);
	}
...
static int __init hetgpu_pacc_mbox_init(void)
{
	...
	if (shared_ddr_size) {
		...
		if (IS_ERR_OR_NULL(g_dentry)) {
			...
			debugfs_create_file("control_write", 0200, g_dentry, NULL,
					    &control_write_debugfs_fops);
		}
	}
